...
const commands: { [key in CommandKey]: Command } = {
  findFiles: {
    key: "findFiles",
  ...
  findInActiveFile: {
    key: "findInActiveFile",
    script: "find_in_active_file",
    uri: undefined,
    preRunCallback: undefined,
    postRunCallback: undefined,
  },
  listSearchLocations: {
    key: "listSearchLocations",
...
interface Config {
  ...
  findInActiveFilePreviewEnabled: boolean;
  findInActiveFilePreviewCommand: string;
  findInActiveFilePreviewWindowConfig: string;
  workspaceSettings: {
    folders: string[];
  };
  canaryFile: string;
...
const CFG: Config = {
  ...
  findInActiveFilePreviewEnabled: true,
  findInActiveFilePreviewCommand: "",
  findInActiveFilePreviewWindowConfig: "",
  workspaceSettings: {
    folders: [],
  },
  canaryFile: "",
...
function createTerminal() {
  const terminalOptions: vscode.TerminalOptions = {
    ...
    env: {
      ...
      FIND_IN_ACTIVE_FILE_PREVIEW_ENABLED: CFG.findInActiveFilePreviewEnabled
        ? "1"
        : "0",
      FIND_IN_ACTIVE_FILE_PREVIEW_COMMAND: CFG.findInActiveFilePreviewCommand,
      FIND_IN_ACTIVE_FILE_PREVIEW_WINDOW_CONFIG:
        CFG.findInActiveFilePreviewWindowConfig,
  ...
  if (CFG.shellArgsForTerminal.length > 0) {
...
function getCommandString(cmd: Command) {
  ...
  if (CFG.useEditorSelectionAsQuery) {
    const editor = vscode.window.activeTextEditor;
    if (editor) {
...
async function executeTerminalCommand(cmd: CommandKey) {
  getIgnoreGlobs();
  if (!CFG.flightCheckPassed && !CFG.disableStartupChecks) {
    if (!reinitialize()) {
      return;
